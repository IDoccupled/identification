...
class CollisionTest:
    def _geometry_id(self, geometry_name):
        ...
        if geom_id < 0 or geom_id >= len(self.collision_model.geometryObjects):
            raise ValueError(
                f"Geometry '{geometry_name}' was not found in collision model"
            )
        return geom_id
    ...
    def __init__(
        ...
        if not urdf_path.is_file():
            raise FileNotFoundError(
                f"\033[91mURDF file not found at {urdf_path}\033[0m"
            )
        if not pkg_dir.is_dir():
            raise NotADirectoryError(
                f"\033[91mPackage directory not found at {pkg_dir}\033[0m"
            )
        ...
        if model:
            self.model = model
            print("\033[92mUsing provided pinocchio model...\033[0m")
        else:
        ...
        print("\033[92mBuilding collision model...\033[0m")
        ...
        print("\033[92mCollision model built.\033[0m")

        if not self.performance:
            for geom in self.collision_model.geometryObjects:
                print(
                    f"  - {geom.name}, parentJoint: {geom.parentJoint}, parentFrame: {geom.parentFrame}"
                )

            print("\033[92mBuilding visual model...\033[0m")
            ...
            print("\033[92mVisual model built.\033[0m")
    ...
    def add_collision_pairs(self, collision_pairs: list[tuple[str, str]]):
        ...
        if self.pair_added:
            raise Exception(
                "Add collision pairs only once before any collision computation"
            )
    ...
    def check_collisions(self, q: np.ndarray, print_time: bool = False):
        ...
        if not self.performance:
            print("\033[92mCollision computation at configuration:\033[0m")
            print(q)
        ...
        for k in range(len(self.collision_model.collisionPairs)):
            try:
                ...
                if result.isCollision():
                    ...
                    print(
                        f"Collision detected for pair {k}: {first_name} <-> {second_name}"
                    ) if not self.performance else None
                    collided = True
                ...
                first_name, second_name = self._pair_geometry_names(k)
                raise RuntimeError(
                    f"Error computing collision for pair {k} ({first_name} <-> {second_name}): {e}"
                )
        print(
            f"Time taken: {(time.time() - start_time) * 1000:.2f} ms"
        ) if print_time else None
...
def main():
    ...
    ct = CollisionTest(
        model=model, urdf_path=URDF_PATH, pkg_dir=PKG_DIR, performance=False
    )
