...
class SDPSolver:
    """
    Pure global SDP-based parameter identification.
    ...
    LMI and per-joint parameter bounds for every joint.
    ...
    def solve(
        ...
        """
        Run **global (all-at-once)** identification.

        All ``dof`` joints are identified simultaneously in a single SDP: one
        SOC per joint over its own torque rows, one 6×6 pseudo-inertia LMI per
        joint, and per-joint parameter bounds.  There is no distal→proximal
        sequencing and no fixed-joint torque compensation — the cross-coupling
        columns between a joint and its (distal) subtree members are handled
        directly because every joint's parameters are free variables of the
        same problem.
        ...
        auto_freeze_threshold : float
            ...
            (per-joint identifiability analysis applied on top of ``bounds``
            before the single solve).
        """
        dof = len(joint_order)
        ...
        # --- Pre-compute the per-joint row masks once: sample k, joint d is
        #     stored at row k·dof + d (joint-major regressor layout). ---
        row_masks = []
        for d in range(dof):
        ...
        if self.verbose:
            print(
                f"Global SDP — all {dof} joints identified together "
                f"(distal→proximal order {joint_order}):"
            )

        # --- Optional auto-freeze (per-joint identifiability on the joint's
        #     own parameter block, same analysis as the sequential version) ---
        if auto_freeze_threshold > 0:
            for d in joint_order:
                prior = pi_prior_list[d]
                Y_blk = Y_stack[row_masks[d]][
                    :, d * N_PER_JOINT : (d + 1) * N_PER_JOINT
                ]
    ...
    def _identify_all(
        ...
        """Single all-joints SDP/SOCP.  Returns (pi_opt, solve_time, λ_list).

        Variables: one 13-param block per joint ``pi[13d:13d+13]`` plus one
        non-negative scalar ``λ_d`` per joint.  Objective ``min Σ_d λ_d``.
        ...
        Constraints per joint ``d``:
          · SOC  ‖Y_d_rows @ pi − τ_d‖₂ ≤ λ_d   (rows of joint d only; the
            ...
            problem, with no need to subtract already-fixed joints)
          · LMI  6×6 pseudo-inertia J_d ≽ eps·I₆  (strict; see ParamBounds)
          · per-parameter bounds / freeze from ``bounds``.
        """
        dof = len(joint_order)
        pi_prior_full = np.concatenate(pi_prior_list)

        # --- Pre-solve diagnostics (verbose): per-joint prior residual, LMI
        #     feasibility and freeze status ---
        if self.verbose:
            for d in joint_order:
                name = joint_names[d] if joint_names else f"joint_{d}"
                prior = pi_prior_list[d]
                lb, ub = bounds.get_bounds_for_joint(d)
                ...
                if not in_bounds:
                    ...
                    for i in viol:
                        print(
                            f"       {_PARAM_LABELS[i]}: prior={prior[i]:.6g}  "
                            f"bounds=[{lb[i]:.6g}, {ub[i]:.6g}]"
                        )
                if frozen_names:
        ...
        for idx, d in enumerate(joint_order):
            ...
            # 3) per-parameter bounds / freeze for joint d
            lb, ub = bounds.get_bounds_for_joint(d)
            for i in range(N_PER_JOINT):
                if bounds.is_frozen(d, i):
                    cstr.append(pi_d[i] == pi_prior_list[d][i])
                else:
                    if np.isfinite(lb[i]):
                        cstr.append(pi_d[i] >= lb[i])
                    if np.isfinite(ub[i]):
                        cstr.append(pi_d[i] <= ub[i])

        # --- Solve ---
        problem = cp.Problem(cp.Minimize(cp.sum(lam)), cstr)
        try:
...
def _build_parser() -> argparse.ArgumentParser:
    ...
    ap.add_argument(
        ...
        default=None,
        metavar="START:END",
        help="[meas] 辨识用时间窗（轨迹是周期的，选一个质量好的周期即可，如 "
        "'10:16.667'；None=全程）",
    )
    ap.add_argument(
        "--no-plot",
