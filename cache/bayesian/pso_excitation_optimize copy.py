...
class PSOoptimizer:
    ...
    def _build_bounds(self):
        ...
        for i in range(dim):
            # Amplitude bounds
            lb[i * n_coeffs_per_joint : i * n_coeffs_per_joint + harmonics * 2] = (
                self.regressor.q_lower_limit[i] + Q_LIMIT_BUFFER
            )
            ub[i * n_coeffs_per_joint : i * n_coeffs_per_joint + harmonics * 2] = (
                self.regressor.q_upper_limit[i] - Q_LIMIT_BUFFER
            )
            # q0 bounds
